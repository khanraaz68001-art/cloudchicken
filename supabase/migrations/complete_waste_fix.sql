-- Complete migration to fix waste management relationships
-- This script safely handles existing functions and recreates everything properly

-- Drop all existing waste-related functions first
...
DROP FUNCTION IF EXISTS public.record_waste(TEXT, UUID, DECIMAL, TEXT, UUID);
...
DO $$
BEGIN
    -- Add foreign key to individual_chickens if table exists
    ...
    -- Add foreign key to butchered_meat if table exists
    ...
    -- Add foreign key to user_profiles if table exists
...
CREATE INDEX IF NOT EXISTS idx_enhanced_waste_records_source_type ON public.enhanced_waste_records(source_type);

-- Create the record_waste function with correct parameter order (no defaults before required params)
...
BEGIN
    ...
    IF p_source_type = 'whole_chicken' THEN
        -- Check if individual_chickens table exists
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'individual_chickens') THEN
            RETURN QUERY SELECT FALSE, 'Individual chickens table does not exist', NULL::UUID;
            RETURN;
        END IF;
    ...
    ELSIF p_source_type = 'butchered_meat' THEN
        -- Check if butchered_meat table exists
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'butchered_meat') THEN
            RETURN QUERY SELECT FALSE, 'Butchered meat table does not exist', NULL::UUID;
            RETURN;
        END IF;
    ...
    SELECT 
        ...
        CASE 
            WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'individual_chickens') 
                 AND (SELECT SUM(current_weight_kg) FROM individual_chickens) > 0 
...
-- Create RLS policies
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON public.enhanced_waste_records;
...
DO $$
BEGIN
    RAISE NOTICE 'Waste management relationships fixed successfully!';
    RAISE NOTICE 'Functions created: record_waste(), get_waste_summary()';
    RAISE NOTICE 'Table created: enhanced_waste_records with proper foreign keys';
END $$;